document.addEventListener('DOMContentLoaded', () => {
    ...
    function analyzeElevenLabsAudio(audio) {
        ...
        try {
            ...
            function analyze() {
                ...
                if (miloAnimation3D) {
                    miloAnimation3D.audioReactivity = normalizedIntensity * 5.0;
                    miloAnimation3D.reactivity = miloAnimation3D.baseReactivity + miloAnimation3D.audioReactivity;
                    
                    if (miloAnimation3D.material && miloAnimation3D.material.uniforms) {
                        miloAnimation3D.material.uniforms.reactivity.value = miloAnimation3D.reactivity;
                    }
                }
    ...
    socket.on("audio_chunk", (data) => {
        audioQueue.push(base64ToBlob(data.audio, `audio/${data.format}`));
    });
    ...
    socket.on("complete", () => {
        ...
        if (audioQueue.length > 0) {
            console.log("🔊 Lecture de l'audio en attente");
            playAudioQueue();
        } else if (isDiscussionModeActive && voiceInterface.classList.contains('active')) {
            // Redémarrer automatiquement la reconnaissance vocale après la réponse
            console.log("🔄 Redémarrage automatique de la reconnaissance après réponse");
            setTimeout(() => {
                startRecognition();
            }, 1500); // Pause un peu plus longue
        }
    });
    ...
    function stopCurrentAudio() {
        ...
        if (miloAnimation3D) {
            miloAnimation3D.audioReactivity = 0;
            miloAnimation3D.reactivity = miloAnimation3D.baseReactivity;
            if (miloAnimation3D.material && miloAnimation3D.material.uniforms) {
                miloAnimation3D.material.uniforms.reactivity.value = miloAnimation3D.reactivity;
            }
        }
    ...
    function playAudioQueue() {
        ...
        currentAudio.onended = () => {
            ...
            if (isDiscussionModeActive && voiceInterface.classList.contains('active')) {
                setTimeout(() => {
                    console.log("🎙️ Redémarrage automatique de la reconnaissance vocale");
                    startRecognition();
